Grootcoin Core version 0.13.2 is now available from:

  <https://download.grootcoin.org/grootcoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/grootcoin-project/grootcoin/issues>
...
updates will be released anymore. Without security updates, using a grootcoin
...
ECDSA signatures inside Grootcoin transactions now use validation using
...
Previous versions of Grootcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Grootcoin Core was to raise the default minimum
relay fee.

Grootcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Grootcoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running grootcoind without having to do any manual configuration.
...
Grootcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Grootcoin Core will relay transactions with
...
entirely in Grootcoin Core 0.13, and it is at this time undecided whether the
...
Grootcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Grootcoin Core automatically creates a hidden service to listen on, without
manual configuration. Grootcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Grootcoin Core is listening, and
...
Grootcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Grootcoin
Core to use floating fees. By default, Grootcoin Core will use floating
fees.
...
At all times, Grootcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) LTC.
Furthermore, Grootcoin Core will never create transactions paying less than
...
`grootcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 29202 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 29202 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /grootcoinrpc>
        ProxyPass http://127.0.0.1:9202/
        ProxyPassReverse http://127.0.0.1:9202/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass grootcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< grootcoinrpc:<password>
    </Location>
...
Restarting grootcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in grootcoin.conf
...
grootcoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/grootcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Grootcoin Core code base has
...
- `grootcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `grootcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Grootcoin scripting system that in combination with
...
Grootcoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Grootcoin Core.
...
The two phases are distinct in the Grootcoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Grootcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Grootcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Grootcoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of grootcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of grootcoins being spent is exactly
...
- **Increased security for multisig:** Grootcoin addresses (both P2PKH addresses
  ...
  believed to be provided by Grootcoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Grootcoin's current ledger (called the UTXO set),
  ...
  note that Grootcoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Grootcoin users to individually opt-in to almost any change in the Grootcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Grootcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Grootcoin
client to Grootcoin Core 0.13.2 to signal for segwit support. When a
...
Since Grootcoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `grootcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `grootcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
