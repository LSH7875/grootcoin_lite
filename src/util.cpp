...
const char * const BITCOIN_CONF_FILENAME = "grootcoin.conf";
const char * const BITCOIN_PID_FILENAME = "grootcoin.pid";
...
#else
    const char* pszModule = "grootcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Grootcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Grootcoin";
#else
    // Unix
    return pathRet / ".grootcoin";
#endif
#endif
}
